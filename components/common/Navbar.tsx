...
const Navbar = () => {
  ...
  const extendedNavItems = [
    ...navItems,
    ...(userRole === "ADMIN" ? [{ key: "6", label: "Dashboard", href: "/admin" }] : [{ key: "7", label: "Profile", href: "/user-profile" }]),
  ]
  return (
    <div className="sticky top-0 z-50 w-full border-b border-border bg-background">
      <div className="container mx-auto flex h-20 w-full items-center justify-between px-4 md:px-6">
        <Link href="/" className="flex items-center gap-2">
          <Image src="/logo.png" alt="AeroCanaan Logo" width={60} height={80} className="object-contain" />
          <p className="mt-4 -ml-6 text-sm font-bold text-foreground">AeroCanaan</p>
        </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden items-center space-x-4 text-sm lg:flex">
            {user ? (
              <>
                <span className="font-bold text-foreground">Welcome, {user.firstName}</span>
                <button
                  onClick={handleLogout}
                  className="rounded-xl bg-primary px-4 py-2 text-sm text-white hover:bg-gray-300 dark:hover:bg-gray-600"
                >
                  Logout
                </button>
              </>
            ...
            <SheetContent side="right" className="w-[300px] bg-background p-6">
              <div className="flex flex-col gap-6 pt-6">
                ...
                <div className="mt-8 flex flex-col gap-4">
                  {user ? (
                    <>
                      <span className="text-center text-foreground">Welcome, {user.firstName}</span>
                      <button
                        onClick={() => {
                          handleLogout()
                          setIsOpen(false)
                        }}
                        className="rounded-lg bg-gray-100 py-3 text-center text-foreground transition-colors duration-200 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700"
                      >
                        Logout
                      </button>
                    </>
                    ...
                    <>
                      <Link
                        href="/login"
                        className="w-full rounded-lg py-3 text-center text-foreground transition-colors duration-200 hover:bg-gray-100 dark:hover:bg-gray-800"
                        onClick={() => setIsOpen(false)}
                      >
                        Log In
                      </Link>
