  ...
  return (
    <main className="min-h-screen bg-background">
      <section className="relative mb-12 h-[40vh] min-h-[300px] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-black/50">
          <div className="max-w-4xl px-4 text-center">
            <h1 className="mb-4 text-3xl font-bold text-white md:text-5xl">Discover Palestine</h1>
            <p className="mx-auto max-w-xl text-sm text-white/90">
              Explore the rich history, breathtaking landscapes, and vibrant culture of Palestine through our curated
              destinations.
            </p>
          </div>
        </div>
        ...
        <TabsContent value="all" className="space-y-8">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {destinations.map((destination) => (
              <DiscoverCard key={destination.id} destination={destination} />
            ))}
          </div>
        </TabsContent>
      ...
      <div className="mx-auto mt-16 rounded-xl bg-accent p-8 text-center">
        ...
        <Link href={"/explore"} className="inline-block rounded bg-[#FA7436] px-4 py-2 text-white hover:bg-[#e56a30]">
          View Our Tours
        </Link>
      </div>
    </main>
